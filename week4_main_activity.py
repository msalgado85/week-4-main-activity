...
print(result[-1])
# print(result[#]) = the number that coresponds with that character, in order to get multiple or a 'word' use the : to mean # through #
print(result.find('string'))
print(result[15:-1])
print(result.find('hancock'))
# since hancock is not in our text, the output of the code above will be -1, meaning does not exist.

#get the index of the third letter from the end of the text
print(result[-3])
print(result[-4])
print(result[-5])
# the negative indicates that youre going from right to left from the end rather than left to right from the beginning
...
SubString = text[2:5]
print(SubString)
#get the first letter all the way to the end letter
print(text[0:])
# 0 is the beginning of the string and if you leave the other side blank it means "all the way to the end

#get the first letter to the final letter and skip every 3rd letter

# Strint start:stop:skip
print(text[0::3])
# Built-in methods:
